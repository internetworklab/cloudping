...
func handleHTTP3Message(key string, stream *quicGo.Stream, cr *pkgconnreg.ConnRegistry, msg []byte) error {
...
func (h *HTTP3Handler) Serve() {
	log.Printf("Serving HTTP3 listener on %s", h.Listener.Addr())
	for {
		...
		log.Printf("Accepted HTTP3 connection: %p %s", conn, conn.RemoteAddr())
		go h.Handle(conn)
	}
}

func (h *HTTP3Handler) Handle(conn *quicGo.Conn) {
	cr := h.Cr
	...
	defer func() {
		log.Printf("Closing HTTP3 connection: %s", remoteKey)
		err := conn.CloseWithError(quicGo.ApplicationErrorCode(quicGo.NoError), "Connection closed by server")
		if err != nil {
			log.Printf("Failed to close HTTP3 connection for %s: %v", remoteKey, err)
		}
	...
	go func(conn *quicGo.Conn) {
		for {
			...
			go func(stream *quicGo.Stream) {
				...
				for scanner.Scan() {
					line := scanner.Bytes()
					if err := handleHTTP3Message(remoteKey, stream, cr, line); err != nil {
