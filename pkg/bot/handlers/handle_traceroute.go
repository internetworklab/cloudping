...
func formatTraceCallbackData(loc pkgbot.LocationDescriptor) string {
	return fmt.Sprintf("trace_location_%s", loc.Id)
}

func parseLocationCodeFromTraceCallbackData(pingCallbackData string) string {
...
func HandleTraceroute(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if update.Message != nil {
		...
		msg, err := b.SendMessage(ctx, &bot.SendMessageParams{
			...
			ReplyMarkup: GetLocationButtons(ctx, locationCode, provider, ctx.Value(CtxKeyTGBtnLayoutCol).(int), formatTraceCallbackData),
		})
		if err != nil {
			log.Printf("failed to send message: %v", err)
		}
...
func HandleTraceQueryCallback(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	activeLocationCode := parseLocationCodeFromTraceCallbackData(update.CallbackQuery.Data)
	...
	_, err = b.EditMessageText(ctx, &bot.EditMessageTextParams{
		...
		ReplyMarkup: GetLocationButtons(ctx, activeLocationCode, provider, ctx.Value(CtxKeyTGBtnLayoutCol).(int), formatTraceCallbackData),
	})
	if err != nil {
		log.Printf("failed to edit message: %v", err)
	}
