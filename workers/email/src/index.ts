...
export default {
	async email(message, env, ctx) {
		...
		const upstreamMail2HTTPAPI = env.UPSTREAM_MAIL_API_URL;
		const senderOfReply = env.REPLY_SENDER_EMAIL;
		const nameOfSenderOfReply = env.REPLY_SENDER_NAME;
		...
		msg.addMessage({
			contentType: upstreamResponse.headers.get('Content-Type')?.replace(/;.+$/, '') ?? 'text/plain',
			data: upstreamText,
		});
